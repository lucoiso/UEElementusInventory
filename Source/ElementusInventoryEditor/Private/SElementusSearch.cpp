...
TSharedRef<SWidget> SElementusSearch::ConstructContent()
{
	...
	const auto CheckBoxCreator_Lambda = [this](const EElementusItemType& InType) -> const TSharedRef<SCheckBox>
	{
		...
		return SNew(SCheckBox).Padding(CheckBoxPadding).OnCheckStateChanged(this, &SElementusSearch::TriggerOnCheckboxStateChanged, Index).Content()
			[
				SNew(STextBlock)
	...
	return SNew(SVerticalBox)
		...
		[
			SNew(SSearchBox).OnTextChanged(this, &SElementusSearch::TriggerOnSearchTextChanged)
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(SlotPadding)
		[
			SNew(STextBlock).Text(FText::FromString(TEXT("Show types:"))).Font(FAppStyle::Get().GetFontStyle("NormalFontBold"))
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(SlotPadding)
		[
			SNew(SUniformGridPanel).SlotPadding(1.f)
