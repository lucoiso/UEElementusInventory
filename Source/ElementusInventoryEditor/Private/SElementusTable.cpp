...
protected:
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName) override
	{
		...
		{
			return SNew(STextBlock).Text(InText).Font(CellFont).Margin(CellMargin).HighlightText(
					HighlightText.IsValid() ? *HighlightText.Get() : FText::GetEmpty());
		};
...
void SElementusTable::Construct([[maybe_unused]] const FArguments&)
{
	...
	const auto HeaderColumnCreator_Lambda = [this](const FName& ColumnId, const FString& ColumnText,
	                                               const float InWidth = 1.f) -> const SHeaderRow::FColumn::FArguments
	{
		return SHeaderRow::Column(ColumnId).DefaultLabel(FText::FromString(ColumnText)).FillWidth(InWidth).
		                                    SortMode(this, &SElementusTable::GetColumnSort, ColumnId).OnSort(this, &SElementusTable::OnColumnSort).
...
{
	return SNew(SElementusItemTableRow, OwnerTable, InItem).Visibility(this, &SElementusTable::GetIsVisible, InItem).HightlightTextSource(SearchText);
}
...
EVisibility SElementusTable::GetIsVisible(const FElementusItemPtr InItem) const
{
	...
	{
		return InText.IsEmpty() || InItem->PrimaryAssetId.ToString().Contains(InText, ESearchCase::IgnoreCase) || FString::FromInt(InItem->Id).
			Contains(InText, ESearchCase::IgnoreCase) || InItem->Name.ToString().Contains(InText, ESearchCase::IgnoreCase) ||
			UElementusInventoryFunctions::ElementusItemEnumTypeToString(InItem->Type).Contains(InText, ESearchCase::IgnoreCase) || InItem->Class.
			ToString().Contains(InText, ESearchCase::IgnoreCase) || FString::SanitizeFloat(InItem->Value).Contains(InText, ESearchCase::IgnoreCase) ||
			FString::SanitizeFloat(InItem->Weight).Contains(InText, ESearchCase::IgnoreCase);
	}(SearchText.IsValid() ? SearchText->ToString() : FString()) && (AllowedTypes.Contains(static_cast<uint8>(InItem->Type)) ||
		UElementusInventoryFunctions::HasEmptyParam(AllowedTypes)))
	{
...
void SElementusTable::UpdateItemList()
{
    ...
    if (UAssetManager* const AssetManager = UAssetManager::GetIfValid();
#endif
		IsValid(AssetManager) && AssetManager->HasInitialScanCompleted() && UElementusInventoryFunctions::HasEmptyParam(ItemArr))
	{
		FMessageDialog::Open(EAppMsgType::Ok,
		                     FText::FromString(TEXT("Asset Manager could not find any Elementus Items. Please check your Asset Manager settings.")));
	}
}
