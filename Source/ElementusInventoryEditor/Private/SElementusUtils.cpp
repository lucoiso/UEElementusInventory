...
TSharedRef<SWidget> SElementusUtils::ConstructContent()
{
	...
	{
		return SNew(SButton).Text(FText::FromString(InStr)).ToolTip(SNew(SToolTip).Text(FText::FromString(Tooltip))).HAlign(HAlign_Center).OnClicked(
				this, &SElementusUtils::OnButtonClicked, ButtonId);
	};

	return SNew(SVerticalBox)
		+ SVerticalBox::Slot().AutoHeight().Padding(SlotPadding)
		[
			SNew(STextBlock).Text(FText::FromString(TEXT("Utils:"))).Font(FAppStyle::Get().GetFontStyle("NormalFontBold"))
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(SlotPadding)
		[
			SNew(SUniformGridPanel).SlotPadding(SlotPadding / 2.f)
