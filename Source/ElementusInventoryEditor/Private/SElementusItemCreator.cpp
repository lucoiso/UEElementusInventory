...
TSharedRef<SWidget> SElementusItemCreator::ConstructContent()
{
	...
	return SNew(SScrollBox)
		+ SScrollBox::Slot()
		[
			SNew(SGridPanel).FillColumn(0, 0.3f).FillColumn(1, 0.7f)
			+ SGridPanel::Slot(0, 0).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("ID"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 0).Padding(SlotPadding)
			[
				SNew(SNumericEntryBox<int32>).AllowSpin(false).MinValue(1).Value_Lambda([this] { return ItemId; }).OnValueChanged_Lambda(
					[this](const int32 InValue)
					{
						ItemId = InValue;
					})
			]
			+ SGridPanel::Slot(0, 1).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Object"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 1).Padding(SlotPadding)
			[
				ObjEntryBoxCreator_Lambda(UObject::StaticClass(), 0)
			]
			+ SGridPanel::Slot(0, 2).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Class"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 2).Padding(SlotPadding)
			[
				SNew(SClassPropertyEntryBox).AllowAbstract(true).SelectedClass(this, &SElementusItemCreator::GetSelectedEntryClass).OnSetClass(
					this, &SElementusItemCreator::HandleNewEntryClassSelected)
			]
			+ SGridPanel::Slot(0, 3).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Name"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 3).Padding(SlotPadding)
			[
				SNew(SEditableTextBox).OnTextChanged_Lambda([this](const FText& InText)
				{
					ItemName = *InText.ToString();
				})
			]
			+ SGridPanel::Slot(0, 4).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Description"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 4).Padding(SlotPadding)
			[
				SNew(SMultiLineEditableTextBox).OnTextChanged_Lambda([this](const FText& InText)
				{
					ItemDescription = InText;
				})
			]
			+ SGridPanel::Slot(0, 5).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Type"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			...
			+ SGridPanel::Slot(0, 6).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Is Stackable"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 6).Padding(SlotPadding)
			[
				SNew(SCheckBox).IsChecked(ECheckBoxState::Checked).OnCheckStateChanged_Lambda([this](const ECheckBoxState InState)
				{
					bIsStackable = InState == ECheckBoxState::Checked;
				})
			]
			+ SGridPanel::Slot(0, 7).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Value"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 7).Padding(SlotPadding)
			[
				SNew(SNumericEntryBox<float>).AllowSpin(false).MinValue(0.0f).Value_Lambda([this] { return ItemValue; }).OnValueChanged_Lambda(
					[this](const float InValue)
					{
						ItemValue = InValue;
					})
			]
			+ SGridPanel::Slot(0, 8).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Weight"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 8).Padding(SlotPadding)
			[
				SNew(SNumericEntryBox<float>).AllowSpin(false).MinValue(0.0f).Value_Lambda([this] { return ItemWeight; }).OnValueChanged_Lambda(
					[this](const float InValue)
					{
						ItemWeight = InValue;
					})
			]
			+ SGridPanel::Slot(0, 9).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Icon"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 9).Padding(SlotPadding)
			[
				ObjEntryBoxCreator_Lambda(UTexture2D::StaticClass(), 1)
			]
			+ SGridPanel::Slot(0, 10).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Image"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 10).Padding(SlotPadding)
			[
				ObjEntryBoxCreator_Lambda(UTexture2D::StaticClass(), 2)
			]
			+ SGridPanel::Slot(0, 11).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Asset Name"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 11).Padding(SlotPadding)
			[
				SNew(SEditableTextBox).OnTextChanged_Lambda([this](const FText& InText)
				{
			...
			+ SGridPanel::Slot(0, 12).Padding(SlotPadding)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Asset Folder"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 12).Padding(SlotPadding)
			[
				SNew(SHorizontalBox)
				+ SHorizontalBox::Slot()
				[
			...
			+ SGridPanel::Slot(1, 13).Padding(SlotPadding * 2.f).HAlign(HAlign_Left)
			[
				SNew(SButton)
				...
				.ToolTip(SNew(SToolTip).Text(FText::FromString(TEXT("Already exists a item with this Id."))).Visibility_Lambda([this]() -> EVisibility
				{
...
void SElementusItemCreator::UpdateFolders()
{
    ...
    if (UAssetManager* const AssetManager = UAssetManager::GetIfValid();
#endif
		IsValid(AssetManager) && AssetManager->HasInitialScanCompleted() && UElementusInventoryFunctions::HasEmptyParam(AssetFoldersArr))
	{
		FMessageDialog::Open(EAppMsgType::Ok,
		                     FText::FromString(TEXT("Asset Manager could not find any folder. Please check your Asset Manager settings.")));
	}
}
...
FReply SElementusItemCreator::HandleCreateItemButtonClicked() const
{
	...
	if (UObject* const NewData = AssetToolsModule.Get().CreateAsset(AssetName.ToString(), FPackageName::GetLongPackagePath(PackageName),
	                                                                UElementusItemData::StaticClass(), Factory))
	{
...
#endif
	{
		return ItemId != 0 && !AssetManager->GetPrimaryAssetPath(FPrimaryAssetId(ElementusItemDataType, *("Item_" + FString::FromInt(ItemId)))).
		                                     IsValid();
	}
