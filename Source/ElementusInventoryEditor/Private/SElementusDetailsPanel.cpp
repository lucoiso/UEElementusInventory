...
void SElementusDetailsPanel::CustomizeHeader(const TSharedRef<IPropertyHandle> PropertyHandle, FDetailWidgetRow& HeaderRow,
                                             IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	PropertyHandlePtr = PropertyHandle;

	HeaderRow.NameContent()[PropertyHandle->CreatePropertyNameWidget()].ValueContent()[SNew(SObjectPropertyEntryBox)
...
void SElementusDetailsPanel::CustomizeChildren(TSharedRef<IPropertyHandle> StructPropertyHandle, IDetailChildrenBuilder& StructBuilder,
                                               IPropertyTypeCustomizationUtils& CustomizationUtils)
{
}
...
void SElementusDetailsPanel::OnObjChanged(const FAssetData& AssetData) const
{
	...
	const FString InValue(FString::Printf(
		TEXT("(PrimaryAssetType=\"%s\",PrimaryAssetName=\"%s\")"), *AssetData.GetPrimaryAssetId().PrimaryAssetType.ToString(),
		...
		if (AssetId.IsValid() && Output.IsEmpty())
		{
			FMessageDialog::Open(EAppMsgType::Ok,
			                     FText::FromString(TEXT(
				                     "Asset Manager could not retrieve asset information: Check if you've added the path to the Asset Manager settings.")));
