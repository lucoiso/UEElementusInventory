...
void FElementusInventoryEditorModule::RegisterMenus()
{
	...
	const TSharedPtr<FWorkspaceItem> Menu = WorkspaceMenu::GetMenuStructure().GetToolsCategory()->AddGroup(
		LOCTEXT("ElementusCategory", "Elementus"), LOCTEXT("ElementusCategoryTooltip", "Elementus Plugins Tabs"),
		FSlateIcon(AppStyleName, "InputBindingEditor.LevelViewport"));
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(ElementusEditorTabId, EditorTabSpawnerDelegate).
	                          SetDisplayName(FText::FromString(TEXT("Elementus Inventory Management"))).SetTooltipText(
		                          FText::FromString(TEXT("Open Elementus Inventory Window"))).SetGroup(Menu.ToSharedRef()).SetIcon(
		                          FSlateIcon(AppStyleName, "Icons.Package"));
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(ItemCreatorTabId, ItemCreatorTabSpawnerDelegate).
	                          SetDisplayName(FText::FromString(TEXT("Elementus Item Creator"))).SetGroup(Menu.ToSharedRef()).SetIcon(
		                          FSlateIcon(AppStyleName, "Icons.PlusCircle"));
}
